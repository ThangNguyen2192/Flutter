...
void checkAll() async {
  for (var element in friends) {
    if (!await checkAssetExists(element.pathImage)) {
      print(element.pathImage);
      element.pathImage = "";
      print(element.pathImage);
      print("v");

      // print(await checkAssetExists(element.pathImage));
    }
  }
...
loadData() {
  ...
  friends = [
    ...
    Friend(name: "friend 2", id: "2", pathImage: "assets/images/21.png"),
...
class Avatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ClipOval(
      child: friend.pathImage.isNotEmpty
          ? Image(
              width: width,
              height: height,
              fit: BoxFit.cover,
          ...
          : Center(
              child: Text(friend.name.toUpperCase().substring(0, 1)),
            ),
    );
  }
}
